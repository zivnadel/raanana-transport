...
const PricesForm: React.FC<any> = ({ initialPrices }) => {
    ...
    const [currentPrices, setCurrentPrices] = useState<PricesObjectType>(initialPrices);
    ...
    const pricesSubmittedHandler = async () => {
        ...
        if (flag) {
            setIsLoading(true)
            const response = await fetch('/api/prices', { method: "PUT", body: JSON.stringify(currentPrices) })
            setIsLoading(false)
            dashboardContext!.action({ type: "setShowPrices", payload: false })
    ...
    return (
        <Modal>
            ...
            {initialPrices && !isLoading && <div className="flex flex-col items-center">
                <h2 className="text-center p-2 text-primary text-2xl">מחירים</h2>
                {Object.entries(initialPrices).map((price: any) => {
                    return <EditableInput onChange={priceChangedHandler} key={price[0]} name={price[0]} type="number" value={price[1].toString()} label={convertToHebrew(price[0])} />
                })}
                {showErrorMessage && <p className="mt-5 text-m text-right text-red-600">!נא למלא מחירים תקינים</p>}
                <Button onClick={pricesSubmittedHandler} className="mt-6 mb-3 p-3 md:w-8/12">אישור</Button>
            </div>}
        </Modal>
    )
}
...
function convertToHebrew(str: string): string {
    switch (str) {
        ...
        case "p8": return "אנשים" + "\u200e" + " 8"
        case "p16": return "אנשים" + "\u200e" + " 16"
        case "p20": return "אנשים" + "\u200e" + " 20"
        case "p23": return "אנשים" + "\u200e" + " 23"
        default: return ""
    }
}
